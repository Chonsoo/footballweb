      ...
      >
        <div className="p-5 pb-3">
          <div className="mb-3 flex items-center gap-3">
            {player.photo_url ? (
              ...
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-white/10 text-sm font-semibold text-neutral-300">
        ...
        <div className="border-y border-white/10 bg-black/10">
          ...
          <div className="min-h-[252px]">
            {loading ? (
              <p className="px-4 py-8 text-center text-sm text-neutral-400">Cargando…</p>
            ) : (
              items.map((it) => (
                <div
                  key={it.key}
                ...
                >
                  <span className="text-center font-medium text-neutral-400">{it.key === 'minutes' ? `${it.count}'` : it.count}</span>
                  <span className="text-neutral-100">{it.label}</span>
                  <span
                    className={`text-right font-semibold ${
                      it.points < 0 ? 'text-red-400' : it.points > 0 ? 'text-green-400' : 'text-neutral-400'
                    }`}
                  >
                    {it.points > 0 ? '+' : ''}
                    {it.points}
